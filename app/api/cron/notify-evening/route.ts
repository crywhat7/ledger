...
export async function POST(request: NextRequest) {
  ...
  for (const sub of subs) {
    try {
      const { weeklyDisposable } = await getDisposableForUser(supabase, sub.user_id, hondurasNow);
      await sendPushNotification(
        ...
        },
        {
          ...
          body: `${format(weeklyDisposable)} para gastar esta semana.`,
          url: "/dashboard",
        }
      );
      sent++;
    } catch (e) {
      console.error("notify-evening for user", sub.user_id, e);
    }
  }
